...
from app.prueba_sql import test_sql_connection as prueba
...
server = st.sidebar.text_input("Servidor SQL", "(localdb)\\Luffy")
database = st.sidebar.text_input("Base de datos", "StreamlitDemo")
username = st.sidebar.text_input("Usuario", None)
password = st.sidebar.text_input("Contraseña", None, type="password")
        ...
        try:
            ...
            with sql_col:
                ...
                with st.spinner("Conectando a SQL Server..."):
                    conn = get_sql_connection(server='(localdb)\\Luffy', database='StreamlitDemo', username=None, password=None)
                ...
                with spark_col:
                    ...
                    with st.spinner("Ejecutando consulta en Spark..."):
                        spark_df, spark_time = execute_spark_query(
                            spark, server, database, None, None, query
                        )
